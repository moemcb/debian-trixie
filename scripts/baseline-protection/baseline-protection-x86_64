#!/bin/bash
###############################################################################
# Core/Media Server Security Hardening Script for Debian 13 Trixie (Generic)
# Designed for x86_64 servers using Cloudflare Tunnels with local network access
#
# NOTE: AMD64 architecture (x86_64) - suitable for media/general servers
#
# Usage: sudo ./vps-harden-core-generic.sh [TUNNEL_IP] [USERNAME] [LOCAL_NETWORK]
# Example: sudo ./vps-harden-core-generic.sh 10.0.0.101 admin 192.168.1.0/24
#
# Parameters:
...
#   LOCAL_NETWORK - Local network CIDR for SSH access (default: 192.168.0.0/24)
#
...
ADMIN_USER="${2:-${SUDO_USER:-admin}}"  # Non-root admin user
LOCAL_NETWORK="${3:-192.168.0.0/24}"  # Local network CIDR
...
LOG_FILE="/var/log/core-hardening-$(date +%Y%m%d-%H%M%S).log"
...
ARCH=$(uname -m)
if [[ "$ARCH" != "x86_64" ]]; then
    log_warning "This script is optimized for x86_64. Detected: $ARCH"
    log_warning "For Raspberry Pi, use vps-harden-pi-generic.sh instead"
...
log "Core Server Hardening Script Starting"
...
# File limits (higher for media streaming)
fs.file-max = 131072
...
###############################################################################
# 7. FIREWALL CONFIGURATION (UFW) - WITH LOCAL NETWORK ACCESS
###############################################################################
...
ufw allow from "$LOCAL_NETWORK" to any port 22 proto tcp comment 'SSH from local network' 2>&1 | tee -a "$LOG_FILE"

log "Additional ports available (uncomment in script as needed):"
log "  - Jellyfin: 8096/tcp"
log "  - Plex: 32400/tcp"
log "  - Emby: 8096/tcp"
log "  - Web UI: 8080/tcp"

# Uncomment these as needed for your media server:
# ufw allow from "$LOCAL_NETWORK" to any port 8096 proto tcp comment 'Jellyfin/Emby'
# ufw allow from "$LOCAL_NETWORK" to any port 32400 proto tcp comment 'Plex'
# ufw allow from "$LOCAL_NETWORK" to any port 8080 proto tcp comment 'Web UI'
...
log "Firewall configured with local network access"
...
# Build ignoreip list
FAIL2BAN_IGNOREIP="127.0.0.1/8 ::1 $LOCAL_NETWORK"
...
log "Fail2ban configured (local network whitelisted)"
...
log "Compiler access restricted to root"
...
# Disable unnecessary services (rsync kept for media sync)
SERVICES_TO_DISABLE=("avahi-daemon" "cups" "bluetooth" "rpcbind")
# Note: rsync NOT disabled - needed for media sync operations
...
###############################################################################
# 17. AIDE CONFIGURATION
###############################################################################
log_section "17. Initializing AIDE"
...
# Full monitoring for core server
...
log "AIDE configured. Run 'aideinit' to initialize database."
...
# Add tunnel IP to hosts.allow if configured
if [[ -n "$TUNNEL_IP" ]]; then
    sed -i "2i sshd: $TUNNEL_IP" /etc/hosts.allow
fi
...
###############################################################################
# 22. SYSTEM LIMITS (Higher for media server)
###############################################################################
...
log "System limits configured (optimized for media streaming)"
...
cat > "$REPORT_FILE" << EOF
================================================================================
Core/Media Server Security Hardening Report
Generated: $(date)
Hostname: $(hostname)
Architecture: $ARCH
================================================================================
...
- SSH: Key-only, tunnel + local network
- Firewall: UFW enabled
- DNS: DNSSEC + DoT (Cloudflare)
- IPv6: Disabled

LOCAL NETWORK SERVICES ENABLED:
- SSH (22/tcp) from $LOCAL_NETWORK

ADDITIONAL PORTS (uncomment in script as needed):
- Jellyfin/Emby: 8096/tcp
- Plex: 32400/tcp
...
2. Test local SSH: ssh $ADMIN_USER@<local-ip>
3. Run AIDE init: sudo aideinit
4. Enable additional ports in UFW as needed
...
log "========================================="
log "CORE SERVER HARDENING COMPLETE!"
log "========================================="
log "Backup: $BACKUP_DIR"
log "Log: $LOG_FILE"
log "Local network: $LOCAL_NETWORK has SSH access"
